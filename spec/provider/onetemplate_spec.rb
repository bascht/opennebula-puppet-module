...
describe provider_class do
  ...
  context 'template handler' do
    ...
    it "should list templates" do
      ...
      provider_class.stubs(:invoke).with('show', 'app', '--xml').returns(response[:show]).times(5)

      template = provider_class.instances.first
    ...
    it "should list existing templates" do
      provider_class.stubs(:invoke).
        with('list', '--xml').returns(response[:list]).times(4)

      provider_class.instances[0].name.should eq 'app'
      provider_class.instances[1].name.should eq 'database'

      provider_class.instances[0].exists?.should be_truthy
      provider_class.instances[1].exists?.should be_truthy

      provider_class.stubs(:invoke).
        with('list', '--xml').returns('<VMTEMPLATE_POOL></VMTEMPLATE_POOL>').once
      provider_class.instances.should eq []
    end
